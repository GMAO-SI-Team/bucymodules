--[[

Built with:

module load llvm-flang/22.1.0

mkdir build-llvm-flang-22.1.0 && cd build-llvm-flang-22.1.0
../configure --disable-wrapper-rpath --disable-wrapper-runpath \
      CC=clang CXX=clang++ FC=flang \
      --with-pmix=internal --with-hwloc=internal --with-libevent=internal \
      --prefix=/ford1/share/gmao_SIteam/MPI/openmpi/5.0.10/llvm-flang-22.1.0 |& tee configure.llvm-flang-22.1.0.log

mv config.log config.llvm-flang-22.1.0.log
make -j4 |& tee make.llvm-flang-22.1.0.log
make install |& tee makeinstall.llvm-flang-22.1.0.log
make check |& tee makecheck.llvm-flang-22.1.0.log

--]]

family("MPI")
prereq("llvm-flang/22.1.0")

local compilername = "llvm-flang-22.1.0"
local version = "5.0.10"
local siteamdir = "/ford1/share/gmao_SIteam"
local pkgdir = pathJoin(siteamdir,"MPI","openmpi",version,compilername)

-- Setup Modulepath for packages built by this MPI stack
local mroot = "/ford1/share/gmao_SIteam/lmodulefiles"
local mdir = pathJoin(mroot,"MPI/llvm-flang-22.1.0",("openmpi-"..version))
prepend_path("MODULEPATH", mdir)

setenv("OPENMPI",pkgdir)
setenv("MPIHOME",pkgdir)
setenv("MPI_HOME",pkgdir)

prepend_path("PATH",pathJoin(pkgdir,"bin"))
prepend_path("LD_LIBRARY_PATH",pathJoin(pkgdir,"lib"))
prepend_path("LIBRARY_PATH",pathJoin(pkgdir,"lib"))
prepend_path("INCLUDE",pathJoin(pkgdir,"include"))
prepend_path("MANPATH",pathJoin(pkgdir,"share/man"))
